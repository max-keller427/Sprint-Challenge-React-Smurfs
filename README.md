...
- [ ] Explain the differences between `client-side routing` and `server-side routing`.
  ...
  Server-side routing: Routing is handled by the server sending information for every request the client makes and refreshes the website to display that information.

  Client-side routing: Routing handled by the javascript downloaded by the client when the webpage initially loads. Routing is handled within the clients browser so the page does not have to reload every time new data is requested, only bits of the page.
...
- [ ] Mention three tools we can use to make AJAX requests.
  ...
  fetch, axios, XMLHttpRequest (xhttp)
